...
| `geo:filter:v1TreeAbsent` | The v1 tree is not on this machine, so oracle **O12** is unreachable. | CI always; **the bridge only in breach of OB-7** | v1 is restored on the target machine, **or** the F1–F18 probes are retired with a reason. Currently broken: PV-129. |
