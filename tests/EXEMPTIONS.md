...
| geo.overlayPolygons | reference | A ring that STRADDLES the frame is drawn whole rather than cut (PV-142). Cutting a filled ring leaves it open, and closing it again means walking the boundary between exit and re-entry - which needs the projected ring PV-137 showed does not always exist. Rings wholly outside ARE dropped. Carried openly: a wrong shape is worse than an honest overhang. Closes when a boundary walk exists that works on a domain-limited projection. |
| geo.overlayTrack | reference | Not clipped to the extent in PV-142, unlike the other four overlays. A track carries a value PER VERTEX, aligned through `info.SourceIndex`; clipping would need the same index through the clip and a value interpolated at each crossing. That is a different problem from cutting a bare polyline and gets its own round. In practice `geo.trackmap` derives the extent FROM the track, so the common path has nothing outside to cut. |
