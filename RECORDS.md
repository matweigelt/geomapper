...
| PV-104 | **The first export of a never-drawn figure is a different image, and every export after it is identical.** Windows passed the two metamorphic tests; headless Linux CI failed both. The instinct is to call that renderer noise and loosen the claim — §4.6 forbids exactly that, so the test was made to *diagnose* instead: export three times, report 1-vs-2 and 2-vs-3. The answer was unambiguous. **1 vs 2: 42 176 of 134 805 pixels differ, 31.29%, max channel delta 254. 2 vs 3: 0 pixels, exactly.** So it is not the renderer being irreproducible; it is the first render differing from every later one. **It hides interactively** because the desktop drains the graphics queue between commands, so a second export always follows an implicit flush — which is why neither this machine nor v1 in four years ever saw it. **It does not hide in `-batch`, on a worker, or in the builder form, where EVERY figure is exported on its first render**: a batch of 200 would write 200 first-render files while anything checked by hand was a second-render one. The file shipped would not be the figure inspected. One `drawnow` before the write, and the claim holds everywhere. |
