...
| G | G.0a | `tools/oracle_sync.py`; O7 **split** (it bundled two claims); O2/O8/O11/O12 corrected; O13–O20 seeded for Stage G | ☑ four disagreements found and closed, gate in CI and `gates.sh` |
| G | G.0b | PV-152 repaired at both causes; `verifyNoTextOverlap`; `geo.internal.textRects`, `labelOverhang` | ☑ **521 + 0 + 0**, worst overlap now **0 pt** against a 0.5 bound, bridge 22-Aug; R-033 |
| G | G.1a | `geo.imageGrid` — the RGB kind, all seven categories | ☑ **532 + 0 + 0**, bridge 22-Aug; R-034 |
| G | G.1b | `geo.basemap` draws an image via one kind branch and an index-map carrier | ☑ **535 + 0 + 0**, bridge 22-Aug; R-034 |
| G | G.1c | `readGrid` image branch (O15); Blue Marble fetch + cache (O14) | ☐ **next** — needs a decision on where the imagery lives |
...
| ~~**OB-7 is BROKEN**~~ | **MET, and it never was broken (PV-148, 22-Aug).** The v1 tree was not gone from the target machine; the test held a stale absolute path and looked one folder too deep. PV-129's diagnosis is **withdrawn**. Against the real tree: **17 of 18 probes reproduced, 0 refuted, 1 blocked** (F17, on O6), and **177 options, 0 unmapped** — V4 and V9 discharged by a run rather than by a record. Suite on the bridge: **516 + 0 + 0, nothing filtered.** | — | — closed |
...
| **O2** | Snyder (1993), *Flattening the Earth* | Published table / formula | The Robinson table itself; the Hammer equal-area modification | Stage B `reference` | ☑ **FILLED for the Robinson half — found 22-Aug-2026 by `oracle_sync.py` on its first run, and it had been filled since Stage B.** `mirror/acceptance.json` carries two frozen criteria whose `source` is this row: the X entries at lat 35 and lat 85. Both refuted the handover's quoted values (PV-005) and the corrected nodes are what the PCHIP reproduces — measured at exactly **0** against a 1e-12 bound. **The Hammer half is NOT filled**, and the row stays narrow about that: the equal-area property is certified by the analytic invariant O3, not by this text. |
...
| **O7** | `gdalwarp -r average` / `gdaldem`, through `mirror/geomap_mirror/gdal_oracle.py` | Second toolkit | **Bilinear and nearest resampling only.** Not mass closure — see O13 | Stage 0 mirror, Stage B `reference` | ☑ **22-Aug-2026, and the row was wrong until this date.** It also named `gdalwarp -r average`, which cannot certify this claim: it takes an unweighted mean of the source centres in a target cell, and on a geographic grid the cell area goes as cos φ, so that is not the cell integral and does not conserve mass away from the equator. `gdal_oracle.py` said so in its header and Part 3 never absorbed it. **The repair is a SPLIT, not a rewording.** This row bundled two claims with different evidence, which is the one-fact-two-definitions shape inside a single table cell: what `gdalwarp` certifies is now this row, and mass closure is O13. A row whose `Certifies` column carries two claims can be half true, and a half-true row reads as a whole-true one. |
...
| **O13** | `cdo remapcon`, or ESMF / `xESMF` | Second toolkit | **Conservative regrid weights and mass closure.** Split out of O7 on 22-Aug-2026, because `gdalwarp -r average` takes an *unweighted* mean of the source centres in a target cell and on a geographic grid cell area goes as cos(phi) — so it is not the cell integral and does not conserve mass away from the equator | Stage B `reference`, `precision` | ☐ Debt **V10**. Until it is filled, mass closure rests on the analytic invariant (O3-class) and on the split/merge metamorphic property — both real, both **narrower** than O7 used to imply. Measured today: 2.60e-14 relative against a 1e-13 bound |
| **O14** | NASA Blue Marble Next Generation, one monthly tile | Real third-party file | The RGB backdrop path: an image read, reprojected and drawn without consuming the data's colour scale | Stage G.1 `reference` | ☐ Package G.1 |
| **O15** | A GeoTIFF with a known geotransform | Real third-party file | `geo.readGrid`'s deferred `.tif` branch: origin, step and row order read from the file rather than assumed | Stage G.1 `reference` | ☐ Package G.1; discharges the GeoTIFF debt deferred at Stage C |
| **O16** | An `shAnalysis` synthesis, evaluated independently at a handful of points | Prior implementation + analytic | That a field crossing the toolbox boundary lands where it claims: latitude convention, longitude registration and units survive the trip | Stage G.2 `reference` | ☐ Package G.2, debt **V13**. Data is present at `E:\DATAPOOL\GravityField\series\itsg\monthly` |
| **O17** | `grdtrack` on an analytically known surface | Second toolkit | `geo.sample`: the value read under a track equals the surface's own value there | Stage G.3 `reference` | ☐ Package G.3 |
| **O18** | The written animation file, read back | Analytic invariant | Frame count, duration and pixel dimensions equal what was asked for | Stage G.4a `contract` | ☐ Package G.4a |
| **O19** | Solid-body rotation | Analytic invariant | `geo.particles`: every tracer returns to its start after exactly one period, and the closure error is the integrator's order, not the projection's | Stage G.4b `precision` | ☐ Package G.4b. Costs nothing and needs no file, which is what makes it the right first oracle for an advection scheme |
| **O20** | A published GMT `.cpt` | Real third-party file | Colour-palette import: parsed and re-emitted unchanged | Stage G.5 `reference` | ☐ Package G.5 |
| **O11** | A published GRACE mascon EWH trend map (e.g. JPL RL06 mascon, any release) | Real scientific product | End-to-end figure sanity: sign, magnitude and pattern of a known signal (Greenland, West Antarctica, north India) | Stage F acceptance | ☐ Matthias to name the release. **CITED-NOT-CONSUMED**: `TestIntegration.m` names this id only in a comment explaining which scenario it cannot yet assert. A citation that says "this oracle is missing" is not consumption, and the gate is told so here rather than in an allow-list of its own — an allow-list would be a second authority and would rot exactly like the register it guarded. |
| **O12** | v1 `geoImagescToolbox` itself, as installed | Prior implementation | **Only** the F1–F18 defect probes (V4). Not an authority on correctness — it is the thing being replaced. | Stage 0 probes | ☑ **FILLED, twice: 15-Aug-2026 and 22-Aug-2026.** 17 of 18 reproduced, 0 refuted, 1 blocked (F17, on O6). The row's own wording stands unchanged and is the reason this ☑ is narrow: v1 is an authority on its own defects and on nothing else |
...
| D-020 | 22-Aug-2026 | **Distribution is git, not a packaged toolbox.** `geoMap.prj` withdrawn; the release checklist's install step becomes *a fresh clone with only the root on the path*. The closure rule is unchanged and its REASON is re-derived: `+geo` may not call into `tests/`, `tools/`, `records/` or `docbuild/` because the **user's path** omits them, where it used to be because the archive did. PV-127 was reproduced with `restoredefaultpath` + `addpath`, which is the user's situation and never was the archive's. | A packaged `.mltbx` | `.mltbx` is deprecated (Matthias, 22-Aug-2026) | MathWorks restores a supported packaging route, **or** a consumer needs an Add-On install rather than a clone |
...
| C-174 | 21-Aug-2026 | **PV-135: the frame band had no width at a point pole.** mollweide, hammer and sinusoidal map the pole to a POINT, so the boundary's top edge had one image, `outwardNormals` left a zero mitre and the ribbon offset by nothing. Coincident vertices are collapsed before the normals; the coincidence tolerance is now relative to the map diagonal, measured 16 orders from the nearest legitimate edge | R-027, PR #28 |
| C-175 | 21-Aug-2026 | **PV-136: the coastline was clipped to the DOMAIN, never to the EXTENT.** 91.8% of the drawn coastline lay outside the frame on the GettingStarted track map. `geo.internal.mapBoundary` promoted out of `geo.frame` as the one authority; `clipToBoundary` cuts at the frame, bisecting in lon/lat to 0.0002 px | R-027, PR #29 |
| C-176 | 21-Aug-2026 | **PV-137: membership belongs in lon/lat, not in a projected ring.** `inpolygon` has no answer where the ring does not exist — an orthographic hemisphere shows a global extent every time. `mapBoundary` reports `Complete = false` rather than throwing | R-027 |
| C-177 | 21-Aug-2026 | **PV-140: registration.** GMT's gridline/pixel, MATLAB's postings/cells, GDAL's geotransform — the concept was missing and its absence was the antimeridian wedge. `geo.grid` carries `Registration`, inferred and overridable; `LonRegion`/`LatRegion` are the area COVERED; `geo.basemap` reports them and draws the surface on cell EDGES. Measured: flat shading paints each face 100% from `CData(i,j)` and never reads the last row or column | R-027 |
| C-178 | 21-Aug-2026 | **`Window = "closed"` on `geo.wrapLongitude` and `geo.project`, for MAP EDGES only.** A global grid's eastern rim is +180 and its western −180; the half-open wrap folds one onto the other. The default is untouched — `acceptance.json` freezes `wrap(180,0) == -180` bitwise, and that is F2's fix | R-027 |
...
| C-191 | 22-Aug-2026 | **PV-145: the frame was drawn on the west side only.** A global extent runs -180..180 since registration, and `geo.project`'s half-open default folds +180 onto -180 — so the ring's eastern side landed on its western one and PV-135's coincident-vertex collapse removed the duplicates. Fourteen boundary projections across `frame.m`, `graticule.m` and `mapBoundary.m` now take `Window = "closed"`; data keeps the half-open default, which is F2's fix | Reported from GettingStarted; measured −2.9108 … −0.0000 |
...
| ~~V15~~ | **The `oracle_sync` self-test mutated the REAL tree, which made its injection unfalsifiable while the tree was dirty.** DISCHARGED 22-Aug-2026. | — | Rebuilt on an **inline fixture** with a silent healthy case, matching the three older gates. Falsified by reintroducing the exact original bug: the self-test now fails with a named message and exit 2, and passes at exit 0 when repaired. | — |
| V13 | The `shAnalysis` ↔ geoMap interface has no test on either side. It was exercised once, by hand, in one session. | Neither repository's suite imports the other, and CI has no gravity data. | **Dischargeable on the bridge, and the data has been named**: `E:\DATAPOOL\GravityField\series\itsg\monthly`. A committed small synthesised fixture gives geoMap a `reference` point that CI can also run; the bridge runs the full comparison against real ITSG solutions and the difference between the two is a registered filter, not a gap. Package G.2, oracle O16. | **Medium** |
| ~~V14~~ | ~~PV-152~~ **DISCHARGED 22-Aug-2026 by G.0b.** Both causes repaired and `GeoMapTestCase.verifyNoTextOverlap` now compares two elements to each other, which nothing in the suite did before. Worst overlap in the showcase front: **0 pt** against a 0.5 bound. | — | Run of 22-Aug, 521 + 0 + 0, R-033. | — |
...
| D-027 | 22-Aug-2026 | **One renderer for both kinds.** `geo.basemap` accepts a `geo.grid` or a `geo.imageGrid`, branching once | A separate `geo.imagebase` element | D-024 separates the *kinds* because a field carries a colour scale and an image does not. It does **not** follow that they need separate *renderers*: the projection, seam roll, rim clamp, edge mesh and flat-shading pad are identical, and duplicating ~200 lines of that geometry to avoid one branch is F6 with extra steps. The carrier is an **index map** — the pseudo-grid's Z is `1:numel`, so `rollToCentre` permutes it and the bands are gathered through the result, applying the seam roll to the picture with the same code that applies it to a field | A third kind arrives, at which point one branch becomes three and the geometry should be extracted instead |
| D-026 | 22-Aug-2026 | **`shAnalysis` routes its visualisation through geoMap.** `shLowLevel.plotSHMap` and its siblings are retired as map renderers | Keeping `plotSHMap` as a dependency-free quick-look; or keeping both silently | Matthias's decision, taken on the finding in 10.8. Two map renderers in one workspace are one fact with two implementations, and they will disagree — `plotSHMap` offers two projections against geoMap's sixteen, its own coastlines, its own colour limits, and none of it certified against PROJ. **The direction matters and is not arbitrary:** the toolbox that draws should be the one that owns drawing, and `shAnalysis` keeps the conventions it alone knows — geocentric latitude, quantity→units, product type — and hands geoMap numbers. Neither imports the other's objects | geoMap becomes unavailable to an `shAnalysis` user — at which point the quick-look has to come back, and the row that removed it says what it was |
