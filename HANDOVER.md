...
| **O5** | Natural Earth 110m coastline `.shp` (real third-party file) | Real data file | Shapefile reader against a file this project did not write | Stage C `reference` | ☐ Matthias to supply |
| **O6** | GSHHG binary `.b`, any resolution (real file) | Real data file | GSHHG reader — **the only thing that can discharge V3** | Stage C `reference` | ☐ Matthias to supply |
