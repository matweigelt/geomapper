...
try
    ...
    if ME.identifier == "geo:grid:AxisNotAngular"
        % NAME THE VARIABLES. geo.grid states the rule; only the reader
        % knows which NetCDF variables it matched, and that is the fact
        % that turns "these are not degrees" into "you matched x and y,
        % and they are eastings and northings" (A-3).
        error('geo:grid:AxisNotAngular', '%s', ME.message + " " + ...
            sprintf(['Read from "%s" using the variables matched as ' ...
                     'longitude and latitude; if that file is on a ' ...
                     'projected grid, pass Lon and Lat explicitly or ' ...
                     'reproject it first.'], source));
    end
